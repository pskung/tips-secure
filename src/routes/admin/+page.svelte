...
>
  <!-- 🌸 แสงสีเรืองรองพื้นหลังเพื่อความสดใสและอบอุ่น -->
  <div class="absolute inset-0 bg-slate-950/80 backdrop-blur-[6px] -z-10"></div>
  <div class="absolute w-[25rem] h-[25rem] rounded-full bg-pink-500/10 blur-[100px] top-12 left-12 -z-10 animate-pulse"></div>
  <div class="absolute w-[20rem] h-[20rem] rounded-full bg-indigo-500/10 blur-[80px] bottom-12 right-12 -z-10 animate-pulse" style="animation-delay: 2s;"></div>

  <!-- 📦 กล่องโดเนทหลักแบบกระจกสะท้อนแสงพรีเมียม -->
  <div class="w-full max-w-lg p-6 md:p-8 space-y-8 bg-slate-900/60 rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] border border-slate-800/80 backdrop-blur-xl text-white transition-all duration-300 hover:border-slate-700/60">
    ...
    <!-- 👑 ข้อมูลผู้รับและหัวเว็บ -->
    <div class="flex flex-col items-center space-y-4 text-center">
      <div class="relative group">
        <div class="absolute -inset-1 rounded-full opacity-60 blur-md transition duration-500 group-hover:opacity-100" style="background: linear-gradient(90deg, {theme.themeColor}, #6366f1, {theme.themeColor});"></div>
        <img 
      ...
      <div class="space-y-1">
        <h1 class="text-3xl font-extrabold tracking-wide" style="color: {theme.themeColor}; text-shadow: 0 4px 12px rgba(0,0,0,0.3);">{theme.vtuberName}</h1>
    ...
    <form onsubmit={handleDonate} class="space-y-5">
      ...
      <button
        ...
        style="background: {cooldownRemaining > 0 ? '#4b5563' : `linear-gradient(135deg, ${theme.themeColor}, #6366f1)`}; box-shadow: 0 8px 24px rgba(99, 102, 241, 0.2);"
      >
        {#if cooldownRemaining > 0}
