...
export async function POST(event: APIEvent) {
  ...
  try {
    ...
    if (origin && origin !== expectedOrigin) {
      ...
      return new Response(JSON.stringify({ error: "Untrusted network origin rejected" }), { status: 403 });
    }
    ...
    if (!is_consented) {
      return new Response(JSON.stringify({ error: "กรุณากดยินยอมยอมรับนโยบายก่อนดำเนินรายการค่ะ" }), { status: 400 });
    }

    if (email_confirm) {
      safeLog("Spam Bot Detected: Invisible honeypot trap triggered.", "WARN", { email_confirm });
      return new Response(JSON.stringify({ error: "Operation rejected" }), { status: 400 });
    }

    if (render_time && (now - Number(render_time) < 1000)) {
      safeLog("Spam Bot Detected: Trigger speed abnormal.", "WARN");
      return new Response(JSON.stringify({ error: "Operation rate limit exceeded" }), { status: 400 });
    }

    if (process.env.TURNSTILE_SECRET_KEY) {
      if (!turnstile_token) {
        return new Response(JSON.stringify({ error: "กรุณารอระบบยืนยันตัวตนสักครู่น้า 🔒" }), { status: 400 });
      }
      ...
      if (!verifyData.success) {
        return new Response(JSON.stringify({ error: "ด่านตรวจพบว่ามีพฤติกรรมเป็นบอท กรุณาลองใหม่อีกครั้งค่ะ" }), { status: 400 });
      }
    }

    const cooldownActive = getCookie(event.nativeEvent, "cooldown_active");
    if (cooldownActive === "true") {
      return new Response(JSON.stringify({ error: "กรุณารอ 1 นาทีก่อนทำรายการถัดไปน้า" }), { status: 429 });
    }

    if (!name || typeof name !== "string" || !/^[a-zA-Z0-9\u0e00-\u0e7f\s._-]+$/.test(name) || name.length < 2 || name.length > 25) {
      return new Response(JSON.stringify({ error: "กรุณาตรวจสอบความถูกต้องของชื่อเล่นค่ะ" }), { status: 400 });
    }
    if (message && (typeof message !== "string" || message.length > 100)) {
      return new Response(JSON.stringify({ error: "ข้อความยาวเกิน 100 ตัวอักษรค่ะ" }), { status: 400 });
    }
    if (isNaN(amount) || amount < 10.00 || amount > 5000.00) {
      return new Response(JSON.stringify({ error: "ยอดเงินโดเนทขั้นต่ำต้องอยู่ระหว่าง 10 - 5,000 บาทค่ะ" }), { status: 400 });
    }

    if (!process.env.XENDIT_SECRET_KEY) {
      return new Response(JSON.stringify({ error: "ระบบยังไม่ได้ตั้งค่าคีย์รับเงินลับหลังบ้านค่ะ" }), { status: 501 });
    }
    ...
    if (!response.ok) {
      ...
      return new Response(JSON.stringify({ error: "ไม่สามารถติดต่อเกตเวย์รับเงินภายนอกได้ค่ะ" }), { status: response.status });
    }
    ...
    return new Response(JSON.stringify({ invoice_url: data.invoice_url }), { status: 200 });

  } catch (error) {
    safeLog("Internal Fatal Exception in Payment Controller", "ERROR", error);
    return new Response(JSON.stringify({ error: "ระบบทำงานหลังบ้านประมวลผลล้มเหลว" }), { status: 500 });
  }
}