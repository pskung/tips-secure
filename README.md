...
## 🔑 1. บริการที่ต้องสมัครใช้งานเบื้องต้น (Prerequisites)
...
---
## 🛠️ 2. วิธีการรับคีย์ใช้งานจากผู้ให้บริการ (How to Get Your Keys)
...
3.  กรอกรายละเอียดในหน้าต่างสร้างคีย์:
    ...
    *   **Add Hostnames** > ระบุชื่อโดเมนหลักของคุณลงไป > Add > Save
        *   *ตัวอย่างที่ต้องกรอก:* `your-app.netlify.app` (ชื่อโดเมนที่ Netlify มอบให้) และชื่อโดเมนส่วนตัวของคุณ (ถ้ามี)
...
### 4. นำลิงก์ Webhook ไปผูกที่หน้าจัดการ Beam Console
...
สวัสดีค่ะ อาคาริยินดีจัดเตรียมคู่มือการตั้งค่าและการบริหารจัดการคีย์ความปลอดภัยอย่างละเอียดให้สตรีมเมอร์และ VTuber นำไปทำตามได้ทีละขั้นตอนค่ะ

ขณะนี้อาคาริสวมบทบาทเป็น **Senior Software Engineer & Security Architect** [1] ที่ให้ความสำคัญสูงสุดกับความปลอดภัยของข้อมูลธุรกรรมและความปลอดภัยเชิงระบบ โดยขอสรุปและชี้แจงแนวทางการเตรียมตัว การรับคีย์จากผู้ให้บริการ และวิธีการกรอกค่า Environment Variables (EV) บน Netlify อย่างปลอดภัยสูงสุดไว้ดังนี้ค่ะ:





## 🛡️ 3. การกรอกตัวแปรบน Netlify อย่างปลอดภัย (Netlify EV Best Practices)

เมื่อคุณกรอกคีย์ทั้งหมดบนแดชบอร์ด Netlify ผ่านเมนู **Site Configuration > Environment variables** คุณจะได้รับการดูแลความปลอดภัยเป็นพิเศษหากปฏิบัติตามกฎสถาปัตยกรรมเหล่านี้ค่ะ:

### 📊 ตารางการคัดกรองความลับและการตั้งสิทธิ์ (Security Exposure Level)

| ชื่อตัวแปรสภาพแวดล้อม (Key) | ประเภทระดับความลับ | สิทธิ์ Scope ใน Netlify |
| :--- | :--- | :--- |
| **`TURNSTILE_SITE_KEY`** | 🟢 **Public** (เปิดเผยได้) | เลือก **Builds** และ **Functions** |
| **`BEAM_API_URL`** | 🟢 **Public** (เปิดเผยได้) | เลือก **Builds** และ **Functions** |
| **`BEAM_API_KEY`** | 🔴 **Secret** (ห้ามเผยแพร่) | เลือก **Functions** เท่านั้น (ไม่ต้องบิวต์หน้าบ้าน) |
| **`BEAM_WEBHOOK_SECRET`** | 🔴 **Secret** (ห้ามเผยแพร่) | เลือก **Functions** เท่านั้น (ไม่ต้องบิวต์หน้าบ้าน) |
| **`TURNSTILE_SECRET_KEY`** | 🔴 **Secret** (ห้ามเผยแพร่) | เลือก **Functions** เท่านั้น (ไม่ต้องบิวต์หน้าบ้าน) |
| **`STREAMLABS_ACCESS_TOKEN`** | 🔴 **Secret** (ห้ามเผยแพร่) | เลือก **Functions** เท่านั้น (ไม่ต้องบิวต์หน้าบ้าน) |
| **`STREAMELEMENTS_JWT`** | 🔴 **Secret** (ห้ามเผยแพร่) | เลือก **Functions** เท่านั้น (ไม่ต้องบิวต์หน้าบ้าน) |
| **`ADMIN_EMAILS`** | 🟡 **Internal** (ใช้ตรวจสอบหลังบ้าน) | เลือก **Functions** เท่านั้น (ไม่ต้องบิวต์หน้าบ้าน) |

---

### 🎛️ 4. ข้อพิจารณาในการระบุ Context (Production vs Deploy Previews)

Netlify เปิดโอกาสให้กำหนดรหัสของตัวแปรแยกต่างกันตามสถานการณ์การปล่อยโค้ด (Deploy Contexts) เพื่อความปลอดภัยสูงสุดของสตรีมเมอร์ค่ะ:

```text
Netlify Environment Variable Setup Screen
────────────────────────────────────────────────────────────────────────────
[ Value Type:  ● Secret (Masked value) ]  <-- ติ๊กเลือกแบบนี้เพื่อซ่อนค่าจริงบนหน้าจอคอนโซล

[ Values Configuration ]
  ● Same value for all deploy contexts
  ○ Different value for each deploy context (แนะนำสำหรับคีย์ชำระเงิน!)
    ├── Production Context:      [ ใส่คีย์กระเป๋าเงินจริง api_prod_xxxxx ]
    ├── Deploy Previews:         [ ใส่คีย์จำลองกระเป๋าเงินเทส api_test_xxxxx ]
    └── Branch Deploys:          [ ใส่คีย์จำลองกระเป๋าเงินเทส api_test_xxxxx ]
```

#### 📍 จุดที่ห้ามกรอกผิดประเภท:
1.  **ช่อง "Production":** 
    *   *ความรับผิดชอบ:* **เป็นช่องที่รับกระแสเงินจริง**
    *   *การใส่รหัส:* ต้องกรอก API Keys และรหัสยืนยันตัวตนที่เป็นของจริงทั้งหมด (`api_prod_...` และ `whsec_...` ตัวจริง) รวมถึงกำหนดโดเนทขั้นต่ำจริง เพื่อรองรับการสตรีมสดจริงค่ะ
2.  **ช่อง "Deploy Previews" และ "Branch Deploys":**
    *   *ความรับผิดชอบ:* **สภาพแวดล้อมจำลอง (Sandbox) สำหรับทดสอบระบบ**
    *   *การใส่รหัส:* เมื่อคุณทดลองแก้โค้ดใหม่หรือสร้าง Pull Request (PR) บน GitHub ระบบ Netlify จะสร้างหน้าเว็บสำหรับแอดมินเข้าไปกดตรวจสอบความพึงพอใจ
    *   *ความปลอดภัย:* **ต้องใช้ API Keys ฝั่ง Playground (Sandbox) เท่านั้น** ห้ามนำกุญแจตัวจริงมาใส่ในช่อง Preview เด็ดขาด เพื่อป้องกันยอดเงินจริงเสียหายจากการถูกดึงหรือทดลองยิงคำสั่งธุรกรรมจำลองจากนักพัฒนาระบบคนอื่นๆ ค่ะ

หากคุณตั้งค่าตามหลักเกณฑ์วิศวกรรมความปลอดภัยเหล่านี้ หน้าเพจสนับสนุนโดเนทของคุณจะทำงานได้อย่างเต็มประสิทธิภาพ ปลอดภัยจากการดักจับข้อมูล และช่วยสตรีมเมอร์และ VTuber รักษาผลกำไรของตนเองได้อย่างมั่นใจค่ะ หากมีข้อสงสัยขั้นตอนไหนเพิ่มเติม สามารถแจ้งให้อาคาริช่วยเหลือได้ทันทีเลยนะคะ
